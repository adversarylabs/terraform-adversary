#!/usr/bin/env node
import { createRequire as __terraformCreateRequire } from 'node:module'; const require = __terraformCreateRequire(import.meta.url);
...
function evaluate(rule, sources, allPaths) {
  ...
  if (match.kind === "block-missing-content") {
    return matchingSources.flatMap(
      (file) => extractBlocks(file.source, match.blockStart).flatMap((block) => {
...
function locateEligible(file, expression, offset = 0, source = file.source, anchors) {
  ...
  for (const match of source.matchAll(new RegExp(expression.pattern, flags))) {
    ...
    const anchor = anchors === void 0 ? eligibleAnchor(file, lineAt(file.source, start), lineAt(file.source, Math.max(start, end - 1))) : eligibleSemanticAnchor(file, match[0], start, anchors);
...
// src/index.ts
function createApp() {
  const app = new Adversary({ name: "terraform", version: "0.0.13", review: { maximumFindings: 8 } });
  registerRules(app);
